...
// https://github.com/peterolson/BigInteger.js/blob/master/README.md
// https://github.com/peterolson/BigRational.js/blob/master/BigRational.js
/*
(C) HERE
*/
...
var bigInt = (function (undefined) {
    ...
    BigInteger.prototype.toString = function (radix) {
        ...
        var sign = this.sign ? "-" : "";
        return '#' + sign + str; // DBG EchoLIsp
    };
    SmallInteger.prototype.toString = function (radix) {
        if (radix === undefined) radix = 10;
        if (radix != 10) return toBase(this, radix);
        return '#' + String(this.value); // DBG EchoLIsp
    };
    ...
    function parseStringValue(v) {
            if (isPrecise(+v)) {
                ...
                // throw "Invalid BigInteger: " + v;
                glisp_error(61,v,"big-int");
            }
            var sign = v[0] === "-";
            if (sign) v = v.slice(1);
            var split = v.split(/e/i);
            if (split.length > 2) // throw new Error("Invalid BigInteger: " + text.join("e"));
            	glisp_error(61,text.join("e"),"big-int::convert");
            ...
            if (split.length === 2) {
                ...
                if (exp !== truncate(exp) || !isPrecise(exp)) 
                	glisp_error(61,exp,"big-int::exp");
                // throw new Error("Invalid BigInteger: " + exp + " is not a valid exponent.");
                ...
                glisp_error(61,exp,"big-int::exponent");
                text += (new Array(exp + 1)).join("0");
                v = text;
            }
            var isValid = /^([0-9][0-9]*)$/.test(v);
            if (!isValid) // throw new Error("Invalid BigInteger: " + v);
            	glisp_error(61,v,"big-int::parse");
    ...
    function parseValue(v) {
        if (typeof v === "number") {
            if( v === truncate(v)) return parseNumberValue(v); // PATCH EchoLisp
            glisp_error(61,v,"big-int::parse-value");
        }
        if (typeof v === "string") {
...
API
--------------------------*/
var _make_big_int = function (b) { // b = str/big-int/js number
	return bigInt(b);
}
...
EchoLisp Interface
*/
function boot_big_int() {
		define_sysfun(new Sysfun ("big-int.make-big-int",_make_big_int,1,1)); 

     	
     	_LIB["big-int.lib"] = true;
     	writeln("BigInteger.lib v1.13 © Peter Olson","color:green");
}
