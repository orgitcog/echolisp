...
G L O B S
******************************/
var  _VERSION = "EchoLisp - 2.4.14 - &beta;" ;
var _DEBUG = 0;
...
var _tty_key_eval = function(key) {
	...
	writeln ("Eval key is : #\\" + 
			String.fromCharCode(_KEY_EVAL) + " ascii: " + _KEY_EVAL, _STYLE.plist["store"]) ;
	return __void;
}
...
* option charset : NYI
-------*/
var _KNOWN_LIBS = ["plot.lib","math.lib","bigint.lib","words.lib","web.lib","struct.lib","timer.lib","dico.fr","dico.fr.tags"];
var _LIB = {}; // loaded
...
function stdin_eval() { // called by [Eval]Key or Eval button 
	if(_PLOTTING || _COMPUTING) { wrileln ("Busy..",_STYLE.plist["warning"]); return; }
	var rc ;
// if(glisp.something === null) {doInit(); writeln("reboot..");} // SAFARI ipad
	...
	if(_UI.tty_echo) 
		writeln(stdin.value ,_STYLE.plist["echo"]); //   'color:gray;font-style:italic;'); // ECHO
...
// htmlEntities("_STYLE_color:red_SPAN_define_SPAN_") -> "<span style='color:red'>define</span>"
function htmlEntities(str) {
    str = str.replace(/</g, '&lt;').replace(/>/g, '&gt;') ; // .replace(/"/g, '&quot;');
    str = str.replace(/_STYLE_(.*?)_SPAN_/g,"<span style='$1'>");
    str = str.replace(/_SPAN_/g,'</span>');
//    str = str.replace(/</g, '&lt;').replace(/>/g, '&gt;') ; // .PATCH FOR DOC
    return str;
}
...
// mask html tags
		...
		var outline = htmlEntities(line); // BOOK NYI
