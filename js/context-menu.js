// https://github.com/theyak/contextmenu


// https://github.com/theyak/contextmenu/blob/master/contextmenu.js
...
( function( window ) {
	...
	var getElementContextMenu = function (elem) {
		var menuref =  elem.dataset.menuref ;
		var menu = MENUS[menuref];
		return menu || null ;
	}
	...
	var onContextMenu = function( e ) {
		...
		target = e.target;
		closeContextMenu();
		var targetContextMenu =  getElementContextMenu(target) ;
	...
	var createContextMenu = function( target ) {
		...
		for ( key in targetContextMenu.menu ) {
			var value = targetContextMenu.menu[ key ];
			...
			MENUS.push (itemContextMenu);
			var menuref = MENUS.length-1;
			item.dataset.menuref = '' + menuref;

			if ( itemContextMenu.enabled ) {
			...
			if ( itemContextMenu.enabled ) {
				if ( isFunction( value.onSelect ) ) {
					itemContextMenu.onSelect = value.onSelect;
				} else {
					itemContextMenu.onSelect = onSelect;
				}

				item.addEventListener( targetContextMenu.event, function( e ) {
					var thisContextMenu = getElementContextMenu(this);
					thisContextMenu.onSelect( target, thisContextMenu.key, item );
					closeContextMenu();
				} );
			}
	...
	var positionContextMenu = function( e, target, menu ) {
		...
		if ( targetContextMenu && targetContextMenu.position ) {
			position = targetContextMenu.position;
		}

		if ( position === "bottom" ) {
			// Display below element, left aligned (what about RTL languages?)
			left = targetBox.left + targetContextMenu.horizontalOffset;
			top = targetBox.bottom + targetContextMenu.verticalOffset;
		} else if ( position === "top" ) {
			// Display above element, left aligned (what about RTL languages?)
			left = targetBox.left + targetContextMenu.horizontalOffset;
			top = targetBox.top - menuBox.height + targetContextMenu.verticalOffset;
		} else if ( position === "right" ) {
			// Display to the right of the element, aligned to the top
			left = targetBox.left + targetBox.width + targetContextMenu.horizontalOffset;
			top = targetBox.top + targetContextMenu.verticalOffset;
		} else if ( position === "left" ) {
			// Display to the left of the element, aligned to the top
			left = targetBox.left - menuBox.width + targetContextMenu.horizontalOffset;
			top = targetBox.top + targetContextMenu.verticalOffset;
		} else {
			// Display wherever the user clicked the mouse
			left = e.clientX + targetContextMenu.horizontalOffset;
			top = e.clientY + targetContextMenu.verticalOffset;
		}
		...
		// Too far to the left?
		if ( left < 0 ) {
			if ( targetContextMenu.horizontalOffset >= 0 ) {
				left = targetContextMenu.horizontalOffset;
			} else {
				left = 0;
			}
		}
		...
		// Too far up?
		if ( top < 0 ) {
			if ( targetContextMenu.verticalOffset >= 0 ) {
				top = targetContextMenu.verticalOffset;
			} else {
				top = 0;
			}
		}
		...
		if ( left + menuBox.width > document.body.clientWidth ) {
			if ( targetContextMenu.horizontalOffset >= 0 ) {
				...
				left = document.body.clientWidth - menuBox.width + targetContextMenu.horizontalOffset;
			}
		}
		...
		if ( top + menuBox.height > document.body.clientHeight ) {
			if ( targetContextMenu.verticalOffset >= 0 ) {
				...
				top = document.body.clientHeight - menuBox.height + targetContextMenu.verticalOffset;
			}
		}
	...
	var setEnabledState = function( selector, key, enabled ) {
		...
		for ( idx = 0; idx < elements.length; idx++ ) {
			menu = getElementContextMenu(elements[idx]) ;
			if ( menu.hasOwnProperty( key ) ) {
				menu[ key ].enabled = enabled;
			}
		}
	};
	...
	var ContextMenu = {
		...
		attach : function( selector, menu, options ) {
			...
			for ( idx = 0; idx < elements.length; idx++ ) {
			// elements[idx].setAttribute("contextMenu",obj);
				elements[idx].dataset.menuref = '' + menuref;
		...
		display : function( e, menu, options ) {
			...
			if ( e instanceof Event ) {
				e.target.dataset.menuref = '' + menuref;
				onContextMenu( e );
			}  else if ( e instanceof HTMLElement ) {
				//e.contextMenu = contextMenu;
				e.dataset.menuref = '' + menuref;
				...
				console.log("*ContextMenu:display", e );
			}
		},
